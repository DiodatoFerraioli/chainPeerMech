...
contract Crypto is IBigNumbers {
    ...
    function verifyProof(bytes memory sVal, bytes memory cVal, bytes memory tVal, bytes[] calldata oldVal, bytes[] calldata newVal, bytes calldata userResult1Val, bytes calldata userResult2Val) external view returns (bool) {
        ...
        if(((g.prepare_modexp(t, prime)).cmp((g.prepare_modexp(s, prime)).modmul((c1_new.modmul(c1_old.mod_inverse(prime, res1), prime)).prepare_modexp(c, prime) , prime), false) == 0) && ((publicKey.prepare_modexp(t, prime)).cmp((publicKey.prepare_modexp(s, prime)).modmul((c2_new.modmul(c2_old.mod_inverse(prime, res2), prime)).prepare_modexp(c, prime), prime), false) == 0)) {
            return true;
        }
        else {
            return false;
